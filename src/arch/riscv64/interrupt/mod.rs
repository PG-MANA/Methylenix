...
impl InterruptManager {
    ...
    pub fn set_timer_interrupt_function(&self, function: fn()) -> Result<(), ()> {
        let interrupt_info = get_interrupt_info_mut();
        let _self_lock = self.lock.lock();
        let _lock = interrupt_info.lock.lock();
        let handler_address = interrupt_info.timer_handler;
        if handler_address != 0 {
            if handler_address != function as _ {
                drop(_lock);
                drop(_self_lock);
                pr_err!("The handler was already registered.");
                Err(())
            } else {
                Ok(())
            }
        } else {
            interrupt_info.timer_handler = function as _;
            cpu::synchronize(&interrupt_info.timer_handler);
            Ok(())
        }
    }
    ...
    pub fn set_device_interrupt_function(
        ...
        let interrupt_info = get_interrupt_info_mut();
        if interrupt_id as usize >= interrupt_info.external_handlers.len() {
            pr_err!("Invalid interrupt id: {:#X}", interrupt_id);
            return Err(());
        }
        let _self_lock = self.lock.lock();
        let _lock = interrupt_info.lock.lock();
        let handler_address = interrupt_info.external_handlers[interrupt_id as usize];
        if handler_address != 0 {
            if handler_address != function as _ {
                drop(_lock);
                drop(_self_lock);
            ...
            interrupt_info.external_handlers[interrupt_id as usize] = function as _;
            cpu::synchronize(&interrupt_info.external_handlers[interrupt_id as usize]);
        }
    ...
    pub fn setup_msi_interrupt(
        ...
        for (i, e) in interrupt_info.external_handlers.iter_mut().enumerate() {
            if *e == 0 {
                *e = function as _;
                cpu::synchronize(e);
    ...
    extern "C" fn interrupt_handler(context_data: *mut ContextData, from_user: bool) {
        use core::mem::transmute;
        let scause = cpu::get_cause();
        ...
        if is_interrupt {
            match reason {
                ...
                cpu::SCAUSE_SUPERVISOR_TIMER_INTERRUPT => {
                    let f = get_interrupt_info_mut().timer_handler;
                    if f != 0 {
                        unsafe { transmute::<usize, fn()>(f)() };
                    } else {
                        pr_warn!("Timer interrupt was not handled.");
                    }
                }
                cpu::SCAUSE_SUPERVISOR_EXTERNAL_INTERRUPT => {
                    ...
                    let handlers = &get_interrupt_info_mut().external_handlers;
                    let f = if (int_id as usize) < handlers.len() {
                        handlers[int_id as usize]
                    } else {
                        0
                    };
                    if f != 0 {
                        if unsafe { transmute::<usize, fn(usize) -> bool>(f)(int_id as usize) } {
                            get_cpu_manager_cluster().interrupt_manager.send_eoi(int_id);
                        }
                    } else {
                        pr_warn!("Interrupt #{int_id} was not handled.");
                    }
                }
                _ => {
                ...
                | cpu::SCAUSE_STORE_PAGE_FAULT => {
                    if from_user {
                        ...
                        kprintln!(
                            "Access Type: {}",
                            match reason {
                                cpu::SCAUSE_INSTRUCTION_PAGE_FAULT => "Instruction Fetch",
                                cpu::SCAUSE_LOAD_PAGE_FAULT => "Data Load",
                                cpu::SCAUSE_STORE_PAGE_FAULT => "Data Store",
                                _ => unreachable!(),
                            }
                        );
                        kprintln!("Target Address: {:#18X}", cpu::get_tval());
