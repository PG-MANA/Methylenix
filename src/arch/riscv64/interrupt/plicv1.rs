...
impl PlatformLevelInterruptController {
    const MAX_MEMORY_MAP_SIZE: MSize = MSize::new(0x4000000);
    ...
    pub fn new(base_address: PAddress, size: MSize, ndev: u32) -> Result<Self, ()> {
        // Round up to the 32bit aligned number
        let number_of_irqs = (((ndev + 1/* IRQ 0 is reserved */) - 1) & !(4 - 1)) + 4;
        if number_of_irqs > Self::MAX_DEVICES {
            pr_err!("The number of irqs is too big: {number_of_irqs}");
            return Err(());
        }

        if size > Self::MAX_MEMORY_MAP_SIZE {
            pr_err!("MMIO area is too big: {size}");
            return Err(());
        }
    ...
    fn read_register(&self, register: usize) -> u32 {
        if self.mmio_size <= MSize::new(register) {
            return 0;
        }
...
impl InterruptController for PlatformLevelInterruptController {
    fn set_priority(&self, interrupt_id: u32, priority: u32) {
        if interrupt_id == 0 || interrupt_id >= self.number_of_irqs {
