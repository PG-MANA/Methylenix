...
impl ContextManager {
    ...
    pub unsafe fn jump_to_context(
        ...
        if allow_interrupt_after_jump {
            context.registers.sstatus &= !cpu::SSTATUS_SIE;
            context.registers.sstatus |= cpu::SSTATUS_SPIE;
        }
    ...
    pub unsafe fn switch_context(
        ...
        if allow_interrupt_after_switch {
            next_context.registers.sstatus &= !cpu::SSTATUS_SIE;
            next_context.registers.sstatus |= cpu::SSTATUS_SPIE;
        }
        old_context.registers.sstatus = cpu::get_sstatus() | cpu::SSTATUS_SPP;
