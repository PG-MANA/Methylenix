...
#[inline(always)]
pub unsafe fn set_atp(atp: u64) {
    unsafe { asm!("csrw satp, {}", in(reg) atp) };
}
...
pub fn get_tval() -> u64 {
    let result: u64;
    unsafe { asm!("csrr {}, stval", out(reg) result) };
    result
}
...
pub fn get_epc() -> u64 {
    let result: u64;
    unsafe { asm!("csrr {}, sepc", out(reg) result) };
    result
}
...
pub fn get_time() -> u64 {
    let result: u64;
    unsafe { asm!("csrr {}, time", out(reg) result) };
    result
}

pub fn flush_data_cache(virtual_address: VAddress, size: MSize) {
    memory_barrier();
    if (get_envcfg() & SENVCFG_CBCFE) != 0 {
