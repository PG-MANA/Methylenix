...
impl Jh7110Timer {
    ...
    pub fn init_with_dtb(&mut self, dtb_manager: &DtbManager, node: &DtbNodeInfo) -> bool {
        ...
        if let Some(interrupt_id) = interrupts.get(cpu::get_hartid() as _) {
    ...
    pub fn start_interrupt(&self) {
        ...
        write_register!(self, timer_interrupt_mask, 1);
        self.clear_interrupt();
        self.reload_timeout_value();
        /* Enable Interrupt and unmask */
        write_register!(self, enable, 1);
        write_register!(self, timer_interrupt_clear, 0);
    }
    ...
    pub fn reload_timeout_value(&self) {
        ...
        cpu::memory_barrier();
        write_register!(self, timer_interrupt_clear, 1);
    }
    ...
    fn interrupt_handler(_interrupt_id: usize) -> bool {
        ...
        if (read_register!(timer, int_status) & (1u32 << cpu::get_hartid() as u8)) == 0 {
