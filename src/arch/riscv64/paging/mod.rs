...
impl PageManager {
    ...
    fn get_target_entry<'a>(
        ...
        if table[index].is_leaf() {
            ...
            assert_ne!(level, 0);
            let next_table = physical_address_to_direct_map(table[index].get_next_table_address())
                .to::<[PageTableEntry; NUM_OF_TABLE_ENTRIES]>();
            self.get_target_entry(
                unsafe { &mut *next_table },
                level - 1,
                virtual_address,
                pm_manager,
            )
    ...
    fn _associate_address(
        ...
        for e in table[index..].iter_mut() {
            ...
            if ((*physical_address & (block_size.to_usize() - 1)) == 0)
                && ((*virtual_address & (block_size.to_usize() - 1)) == 0)
                && (*size >= block_size)
            {
                /* Leaf Entry */
                e.init();
                ...
                *virtual_address += block_size;
                ...
                let next_table = next_table_address.to::<[PageTableEntry; NUM_OF_TABLE_ENTRIES]>();
    ...
    fn _cleanup_page_tables(
        ...
        if table[index].has_next() {
            ...
            let next_table = physical_address_to_direct_map(next_table_address)
                .to::<[PageTableEntry; NUM_OF_TABLE_ENTRIES]>();
    ...
    fn _dump_table(
        ...
        let print_omitted = |v: VAddress, p: PAddress| {
            kprintln!(
                "... {:>#18X}        {:>#18X} (fin)",
                v.to_usize().wrapping_sub(1),
                p.to_usize().wrapping_sub(1)
            );
        };
        let block_size = Self::get_block_size(level);
        for e in table {
            ...
            if e.is_leaf() {
                ...
                *virtual_address += block_size;
                ...
                let next_table = physical_address_to_direct_map(e.get_next_table_address())
                    .to::<[PageTableEntry; NUM_OF_TABLE_ENTRIES]>();
                self._dump_table(
                    start_address,
                    end_address,
