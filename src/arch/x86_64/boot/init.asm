...
init:
        ;;「boot」と出してみる。
        ;mov        dword   [0xb8000],0x0b6f0b42;0b:黒地にシアン 順序注意(エンディアン)
        ;mov        dword   [0xb8004],0x0b740b6f
        ;処理
        ...
        ;TSSセグメント情報書き込み
        mov eax,tss
        mov  word [gdt.tss_addr + 2],ax
        shr eax,16
        mov byte [gdt.tss_addr + 4],al
        mov byte [gdt.tss_addr + 7],ah
        ...
        mov     eax,0x80000001
        cpuid
        test    edx,1<<29;LMビットをテスト(Intel64が有効かどうか)
        jz      x86
;ページング設定
;1GBページング対応かどうか
        test    edx,1<<26
...
;setup_64:
        ...
        wrmsr
        mov     eax,cr0
        or          eax,1<<31 | 1;PGフラグを立てる(|1は既に32bitになってる場合は不要)
        mov     cr0,eax
        ;これらの初期化で1GBは仮想メモリアドレスと実メモリアドレスが一致しているはず。(ストレートマッピング)
        lgdt        [gdtr0]
        mov     ax,gdt.tss
...
gdt:
    ...
    .main_code:equ $ - gdt
        dq      (1<<43) | (1<<44) | (1<<47) | (1<<53);すべてコードセグメント
...
align   4096
tss:
tss_addr:equ $ ;tss_addrをうまく使えないか
    dd      0
    dd      stack
    dd      0
    times   22                      dd  0
