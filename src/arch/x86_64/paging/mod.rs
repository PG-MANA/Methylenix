...
impl PageManager {
    pub fn new(pm_manager: &mut PhysicalMemoryManager) -> Option<PageManager> {
        let mut p: PageManager = PageManager { pml4: 0 };
        if p.init(pm_manager) {
            Some(p)
        } else {
            None
        }
    ...
    pub fn init(&mut self, pm_manager: &mut PhysicalMemoryManager) -> bool {
        if let Some(pml4_address) = pm_manager.alloc(PAGE_SIZE, true) {
            self.pml4 = pml4_address;
            let pml4_table = unsafe { &mut *(self.pml4 as *mut [PML4; PML4_MAX_ENTRY]) };
            for pml4 in pml4_table.iter_mut() {
                pml4.init();
            }
            self.associate_address(pm_manager, pml4_address, pml4_address, MemoryPermissionFlags::data());
            true
        } else {
            false
        }
    ...
    fn get_target_pte(&mut self, pm_manager: &mut PhysicalMemoryManager, linear_address: usize, should_set_present: bool) -> Option<&'static mut PTE> {
        ...
        if !pml4_table[number_of_pml4].is_pdpe_set() {
            if let Some(address) = pm_manager.alloc(PAGE_SIZE, true) {
                ...
                self.associate_address(pm_manager, address, address, MemoryPermissionFlags::data());
            } else {
                return None;
            }
        }
    ...
    pub fn associate_address(&mut self, pm_manager: &mut PhysicalMemoryManager, physical_address: usize, linear_address: usize, permission: MemoryPermissionFlags) -> bool {
        /*物理アドレスと理論アドレスを結びつける*/
        if ((physical_address & !PAGE_MASK) != 0) || ((linear_address & !PAGE_MASK) != 0) {
            return false;
        }

        if let Some(pte) = self.get_target_pte(pm_manager, linear_address, true) {
    ...
    pub fn unassociate_address(&mut self, linear_address: usize, pm_manager: &mut PhysicalMemoryManager) -> bool {
        if let Some(pte) = self.get_target_pte(pm_manager, linear_address, false) {
            pte.set_addr(0);
