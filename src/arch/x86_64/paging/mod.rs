...
impl PageManager {
    ...
    pub fn init(&mut self, cache_memory_list: &mut FreePageList) -> bool {
        ...
        self.associate_address(cache_memory_list, pml4_address, pml4_address, MemoryPermissionFlags::data())
    }

    fn get_target_pte(&mut self, cache_memory_list: &mut FreePageList, linear_address: usize, should_set_present: bool) -> Option<&'static mut PTE> {
        ...
        if !pml4_table[number_of_pml4].is_pdpe_set() {
            ...
            if !self.associate_address(cache_memory_list, address, address, MemoryPermissionFlags::data()) {
                return None;
            }
        }
    ...
    pub fn associate_address(&mut self, cache_memory_list: &mut FreePageList, physical_address: usize, linear_address: usize, permission: MemoryPermissionFlags) -> bool {
        /*物理アドレスと理論アドレスを結びつける*/
