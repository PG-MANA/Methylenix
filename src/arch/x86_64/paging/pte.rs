...
impl PTE {
    pub fn new(addr: usize) -> PTE {
        let mut result = PTE::new_empty();
        result.set_addr(&Frame::makefromaddr(addr));
        result
    }
    ...
    pub fn set_addr(&mut self, page: &Frame) {
        assert!(page.start_addr() & !0xFFFFFFFFFF000 == 0);
        self.flags &= (!(0xFFFFFFFFFF000) | page.start_addr()) as u64;
    }
