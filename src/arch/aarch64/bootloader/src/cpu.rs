...
pub fn get_current_el() -> u64 {
    let result: u64;
    unsafe { asm!("mrs {:x}, CurrentEL", out(reg) result) };
    result
}
...
pub fn get_id_aa64mmfr1_el1() -> u64 {
    let result: u64;
    unsafe { asm!("mrs {:x}, id_aa64mmfr1_el1", out(reg) result) };
    result
}
...
pub fn get_tcr_el2() -> u64 {
    let result: u64;
    unsafe { asm!("mrs {:x}, tcr_el2", out(reg) result) };
    result
}
...
pub fn get_hcr_el2() -> u64 {
    let result: u64;
    unsafe { asm!("mrs {:x}, hcr_el2", out(reg) result) };
    result
}
