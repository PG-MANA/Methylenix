...
impl Fatal {
    pub fn parse(stream: &mut AmlStream, current_scope: &NameString) -> Result<Self, AmlError> {
        ...
        let fatal_arg = TermArg::parse_integer(stream, current_scope)?;
        Ok(Self {
            fatal_type,
            fatal_code,
            fatal_arg,
...
impl IfElse {
    fn parse(stream: &mut AmlStream, current_scope: &NameString) -> Result<Self, AmlError> {
        ...
        let predicate = TermArg::parse_integer(&mut if_scope_stream, current_scope)?;
...
impl Notify {
    pub fn parse(stream: &mut AmlStream, current_scope: &NameString) -> Result<Self, AmlError> {
        /* NotifyOp was read */
        let notify_object = SuperName::parse(stream, current_scope)?;
        let notify_value = TermArg::parse_integer(stream, current_scope)?;
        Ok(Self {
            notify_object,
            notify_value,
        })
...
impl While {
    fn parse(stream: &mut AmlStream, current_scope: &NameString) -> Result<Self, AmlError> {
        ...
        let predicate = TermArg::parse_integer(&mut while_scope_stream, current_scope)?;
    ...
    pub fn try_parse(stream: &mut AmlStream, current_scope: &NameString) -> Result<Self, AmlError> {
        ...
        match stream.read_byte()? {
            opcode::EXT_OP_PREFIX => match stream.read_byte()? {
                opcode::SIGNAL_OP => Ok(Self::DefSignal(SuperName::parse(stream, current_scope)?)),
                opcode::RESET_OP => Ok(Self::DefReset(SuperName::parse(stream, current_scope)?)),
                opcode::RELEASE_OP => {
                    Ok(Self::DefRelease(SuperName::parse(stream, current_scope)?))
                }
                opcode::FATAL_OP => Ok(Self::DefFatal(Fatal::parse(stream, current_scope)?)),
            ...
            opcode::IF_OP => Ok(Self::DefIfElse(IfElse::parse(stream, current_scope)?)),
            opcode::NOOP_OP => Ok(Self::DefNoop),
            opcode::NOTIFY_OP => Ok(Self::DefNotify(Notify::parse(stream, current_scope)?)),
            opcode::RETURN_OP => Ok(Self::DefReturn(TermArg::parse(stream, current_scope)?)),
            opcode::WHILE_OP => Ok(Self::DefWhile(While::parse(stream, current_scope)?)),
            _ => {
                stream.roll_back(&back_up);
                Err(AmlError::InvalidType)
            }
